...
func deletePost(id string, user string) error {
    ...
    if posts := getPostFromSearchResult(searchResult); posts == nil {
        fmt.Println("You can only delete your own post!")
        return errors.New("Post not found in Elasticsearch!")
    }

    err = deleteFromES(query, POST_INDEX)
    if err != nil {
        return err
    }

    return deleteFromGCS(id)
}
